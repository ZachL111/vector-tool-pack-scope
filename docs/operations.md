...
`vector-tool-pack-scope` is centered on cli tools work in Python. The technical target is: Package a Python local lab for pack analysis with round-trip fixtures, lossless normalization checks, and documented operating limits.
...
The score is deterministic. The current threshold is 176. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 3.
